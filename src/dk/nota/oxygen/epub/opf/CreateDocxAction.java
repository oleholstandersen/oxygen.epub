...
public class CreateDocxAction extends ArchiveSensitiveAction {
	...
	public void actionPerformed(EditorAccess editorAccess) {
		...
		File docxFile = editorAccess.getWorkspace().chooseFile(new File(
				epubAccess.getArchiveFileUrl().toURI().resolve(docxFileName)),
				"Export [Docx]", new String[] {"docx"}, "Word documents",
				true);
		if (docxFile == null) return;
		CreateDocxWorker createDocxWorker = new CreateDocxWorker(editorAccess,
				epubAccess, new ConsoleWindow("Create Docx"), docxFile);
